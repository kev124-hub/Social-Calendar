...
| `google.test.mjs` | The Google Calendar payload conversion, both directions. All-day dates across six timezones, Google's exclusive all-day `end.date`, year-roll and leap-day boundaries, and the echo filter that stops the pull re-importing what the push just sent. Since the timezone work: an all-day event pushes as the same date from five pushing zones, a timed event survives push→pull with its instant *and* its zone, and a structural tripwire guards the plan's binding constraint — the `source === 'app'` check and the count of calendar-mutating requests, so a new push path cannot be added without re-reading the guarantee that this never modifies a Google event. |
| `ig-token.test.mjs` | `src/lib/ig-token-policy.ts`, Instagram token upkeep. The 23 Sept 2026 outage: a "refresh" that returns the same expiry (to the millisecond, or a few seconds off) is not a refresh; a changed env token wins over a stale DB row, but a first sighting only records a baseline; every warning carries the full manual steps, including the extend step, and no database step. |
...
| `zone-list.test.mjs` | The zone picker's ordering and search — step 4's open question was whether 418 zones are usable on a phone. Recents lead, then the device, then the rest, deduplicated and capped; `new york` with a space matches `America/New_York`; `UTC` is offered even though `Intl.supportedValuesOf` omits it; and a legacy alias like `US/Eastern` stays selectable because validity is what `Intl` accepts, not membership of the canonical list. |
